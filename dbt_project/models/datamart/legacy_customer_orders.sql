...
from raw_data.jaffle_shop.orders as orders

join (
      select 
        first_name || ' ' || last_name as name, 
        * 
      from raw_data.jaffle_shop.customers
) customers
...
join (
    ...
    from (
      select 
        ...
        *
      from raw_data.jaffle_shop.orders
    ) a
    ...
    left outer join raw_data.stripe.payment c
    on a.id = c.orderid
...
left outer join raw_data.stripe.payment payments
