...
-- create DBT user
drop user if exists identifier($dbt_username);
create user identifier($dbt_username)
...
create warehouse identifier($airbyte_warehouse)
...
create warehouse identifier($dbt_warehouse)
warehouse_size = xsmall
warehouse_type = standard
auto_suspend = 30
auto_resume = true
...
drop database if exists identifier($raw_data_database);
create database identifier($raw_data_database);

-- create DBT databases
drop database if exists identifier($dbt_staging_database);
create database identifier($dbt_staging_database);
drop database if exists identifier($dbt_datamart_database);
create database identifier($dbt_datamart_database);
...
-- Grant access to Airbyte to ingest Raw Data
GRANT USAGE ON SCHEMA identifier($raw_data_schema) to ROLE identifier($airbyte_role);
GRANT CREATE TABLE ON SCHEMA identifier($raw_data_schema) TO ROLE identifier($airbyte_role);
